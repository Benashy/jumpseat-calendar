...
select cron.schedule(
  ...
  select
    net.http_post(
      url := (select decrypted_secret from vault.decrypted_secrets where name = 'opsdeck_project_url') || '/functions/v1/opsdeck-telegram',
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'x-opsdeck-cron-secret', (select decrypted_secret from vault.decrypted_secrets where name = 'opsdeck_cron_secret')
      ),
